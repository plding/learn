...
void warpTriangle(Mat &img1, Mat &img2, vector<Point2f> &triangle1, vector<Point2f> &triangle2)
{
    ...
    multiply(img2(rectangle2), Scalar::all(1.0) - mask, img2(rectangle2));
...
int main(int argc, char **argv)
{
    ...
    size_t numswaps = min(shape1.size(), shape2.size());
    for (size_t z = 0; z < numswaps; ++z) {
        ...
        fillConvexPoly(mask, &hull[0], (int) hull.size(), Scalar::all(255));
        // Clone seamlessly
